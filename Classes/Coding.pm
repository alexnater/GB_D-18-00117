...
#			my ($change, $poly, $basecodon, $altcodon, $baseaa, $altaa, @triplet);
			CODON: while ( length($refseq)>=3 ){
				...
				next CODON unless $poly;	# next codon if codon invalid or without polymorphic positions.

				$basecodon=~tr/ATGC/TACG/ if ($dir eq '-');
				my $baseaa=$ref_codon_hash->{$basecodon};
				...
				if ($baseaa eq '_' || $altaa eq '_'){
					...
					if (@positions){
						$change='nonsense';
						warn "Warning, premature stop codon ($baseaa/$altaa) detected at $chrom:$codstart-$codend! CDS end at $end.\n";
						}
					}
